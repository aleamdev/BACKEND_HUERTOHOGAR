...
public class GlobalExceptionHandler {
    ...
    // Errores de validación en parámetros (@PathVariable / @RequestParam)
    ...
    // Usuario no encontrado en login / seguridad
    ...
    // Nuestras RuntimeException tipo "Producto no encontrado"
    @ExceptionHandler(RuntimeException.class)
    public ResponseEntity<ApiError> handleRuntimeException(
            RuntimeException ex,
            HttpServletRequest request
    ) {
        // Si quieres puedes diferenciar según el mensaje,
        // pero para simpleza todo 400.
        ApiError error = buildError(HttpStatus.BAD_REQUEST, ex.getMessage(), request.getRequestURI());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
    }
